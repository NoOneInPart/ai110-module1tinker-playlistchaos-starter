...
def profile_sidebar():
    ...
    # Cap the Chill ceiling below the Hype floor so the bands can never
    # overlap, leaving at least one energy level for the Mixed bucket.
    chill_ceiling = max(1, hype_min - 1)
    profile["chill_max_energy"] = st.sidebar.slider(
        "Chill max energy",
        min_value=1,
        max_value=chill_ceiling,
        value=min(int(profile.get("chill_max_energy", 3)), chill_ceiling),
    )
...
def add_song_sidebar():
    ...
    if st.sidebar.button("Add to playlist"):
        ...
        if title and artist:
            ...
            #claude says this prevents duplicate songs ok

            is_duplicate = any(
                existing.get("title") == normalized["title"]
                and existing.get("artist") == normalized["artist"]
                for existing in all_songs
            )
            if is_duplicate:
                st.sidebar.warning(
                    f"\"{normalized['title']}\" by {normalized['artist']} "
                    f"is already in the playlist."
                )
            else:

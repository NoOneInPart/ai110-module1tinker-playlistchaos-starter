...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    # If thresholds cross, collapse the overlap so a song can't match both
    # the Hype floor and the Chill ceiling (which would make the result
    # depend on check order rather than intent).
    if chill_max_energy >= hype_min_energy:
        chill_max_energy = hype_min_energy - 1
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    # instead of getting a total of all hype songs, get the total of all songs
    # when calculating hype percentage
    total = len(all_songs)
